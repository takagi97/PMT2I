...
prompt_v33 = '''###shot_n_text###Translate the following text into ###target_language###. Your translation should be authentic, specific and easy to understand. Only the translation results are required.\n###input###'''
...
if __name__ == "__main__":
    ...
    src_sents = []
    with open(args.file_need_translate_path, 'r') as f:
        for i in f:
            content = json.loads(i)
            src_sents.append(content)
    ...
    if language == 'French':
        ...
        shots = [{'src': 'French electoral law rather strictly codifies the proceedings.', 
                'tgt': 'Das franz?sische Wahlgesetz legt den Ablauf ziemlich streng fest.'},
                {'src':'The result of plotting analysis will be posted to a public website.',
                 'tgt':'Das Ergebnis der Plotting-Analyse wird auf einer ?ffentlichen Website publiziert.'},
                {'src':'The first cases of the disease this season were reported in late July.',
                 'tgt':'Die ersten F?lle der Krankheit dieser Saison wurden Ende Juli gemeldet.'},
    ...
    if os.path.exists(os.path.join(args.result_data_path, "result.json")):
        with open(os.path.join(args.result_data_path, "result.json"), "r",encoding='utf-8', errors='ignore') as fin:
            for line in fin:
                line = line.strip()
    ...
    fout_prompt_writer = open(f'{args.result_data_path}/{tgt_lang_flores}_prompt.json','a',encoding='utf-8', errors='ignore')
    fout_writer = open(f'{args.result_data_path}/{tgt_lang_flores}_result.json','a',encoding='utf-8', errors='ignore')
    ...
    while len(machine_data) < len(src_sents):
        ...
        fout_prompt_writer.write(json.dumps(font_prompt_inner_dict,ensure_ascii=False) + '\n')
        ...
        fout_writer.write(json.dumps(font_inner_dict,ensure_ascii=False) + '\n')
